# #!/usr/bin/env python3
# -*- coding: utf-8 -*-
from .InputOutput import DWSaver, DWLoader
...
class DWWaterDetect:
    ...
    def necessary_bands(self, include_rgb):
        ...
        necessary_bands = {'Green', 'Red', 'Blue', 'Nir',
                           'Mir2', 'Mir', self.config.reference_band}

        if include_rgb:
            necessary_bands = necessary_bands.union({'Red', 'Green', 'Blue'})

        bands_cluster_set = [
            item for sublist in self.config.clustering_bands for item in sublist]
        bands_graphs_set = [
            item for sublist in self.config.graphs_bands for item in sublist]

        necessary_bands = necessary_bands.union(
            bands_cluster_set).union(bands_graphs_set)
    ...
    def calc_nd_index(self, index_name, band1, band2, save_index=False):
        ...
        if save_index:
            self.saver.save_array(
                index, self.loader.current_image_name + '_' + index_name, no_data_value=-9999)
    ...
    def calc_m_nd_index(self, index_name, band1, band2, band3, band4, save_index=False):
        ...
        index, mask = DWutils.calc_normalized_difference(
            first, second, self.loader.invalid_mask)
        self.loader.update_mask(mask)
    ...
    def calc_mbwi(self, bands, factor=3, save_index=False):
        ...
        if save_index:
            self.saver.save_array(
                mbwi.filled(), self.loader.current_image_name + '_mbwi', no_data_value=-9999)
    ...
    def calc_awei(self, bands, save_index=False):
        ...
        if save_index:
            self.saver.save_array(
                awei.filled(), self.loader.current_image_name + '_awei', no_data_value=-9999)
    ...
    def _detect_water(self, post_callback=None) -> None:
        ...
        for image in self.loader:
            ...
            try:
                ...
                self.saver.set_output_folder(
                    image.current_image_name, image.geo_transform, image.projection)
                ...
                if image.shape_file:
                    ...
                    if post_callback is None:
                        ...
                        image.clip_bands(image.product_dict['bands_names'].keys(), self.config.reference_band,
                                         self.saver.temp_dir)

                    self.saver.update_geo_transform(
                        image.geo_transform, image.projection)
                ...
                invalid_pct = np.count_nonzero(
                    image.invalid_mask) / image.invalid_mask.size
                if invalid_pct > self.config.maximum_invalid:
                    print(
                        f'Invalid pixels ({invalid_pct}) > maximum ({self.config.maximum_invalid}). Skipping image')
                    continue
                else:
                    print(
                        f'Invalid pixels ({invalid_pct}) < maximum ({self.config.maximum_invalid}).')
                ...
                if self.config.average_results:
                    print(
                        'Calculating water mask considering the average of combinations.')
    ...
    def test_pekel(self, image, dw_image, pdf_merger_image):
        ...
        result = jaccard_score(
            pekel_mask > self.config.pekel_water, water_mask == 1) * 100
        ...
        pdf_merger_image.append(DWutils.write_pdf(
    ...
    def create_colorbar_pdf(self, param_name, colormap, min_value, max_value, units=''):
        ...
        filename = self.saver.output_folder.joinpath(
            'colorbar_' + param_name + '.pdf')
    ...
    def create_rgb_burn_in_pdf(self, product_name, burn_in_arrays, colors=None, min_value=None, max_value=None,
        ...
        pdf_filename = DWutils.tif_2_pdf(
            filename, self.config.pdf_resolution, scale=10000)
    ...
    def calc_indexes(self, image, indexes_list, save_index=False):
        ...
        if 'mndwi' in indexes_list:
            ...
            self.calc_nd_index(
                'mndwi', raster_bands['Green'], raster_bands['Mir'], save_index=save_index)

        if 'ndwi' in indexes_list:
            # calculate the NDWI update the mask and saves it
            self.calc_nd_index(
                'ndwi', raster_bands['Green'], raster_bands['Nir'], save_index=save_index)

        if 'ndvi' in indexes_list:
            # calculate the NDVI update the mask and saves it
            self.calc_nd_index(
                'ndvi', raster_bands['Nir'], raster_bands['Red'], save_index=save_index)
        ...
        self.saver.save_array(
            image.invalid_mask, image.current_image_name + '_invalid_mask', dtype=gdal.GDT_Byte)

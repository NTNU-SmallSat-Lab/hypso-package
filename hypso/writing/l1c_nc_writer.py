...
def l1c_nc_writer(satobj: Hypso, dst_nc: str, datacube: str = False) -> None:
    ...
    with (nc.Dataset(dst_nc, 'w', format='NETCDF4') as netfile):
        ...
        try:
            ...
            print("[WARNING] Unable to write indirect latitude and longitude information to NetCDF file. Please run indirect georeferencing if you would like to include these fields.")
        ...
        try:
            ...
            latitude_direct = netfile.createVariable(
                'navigation/latitude_direct', 'f4', ('lines', 'samples'),
                # compression=COMP_SCHEME,
                # complevel=COMP_LEVEL,
                # shuffle=COMP_SHUFFLE,
            )
            latitude_direct[:] = satobj.latitudes
            latitude_direct.long_name = "Latitude (Direct Georeferencing)"
            ...
            longitude_direct = netfile.createVariable(
                'navigation/longitude_direct', 'f4', ('lines', 'samples'),
                # compression=COMP_SCHEME,
                # complevel=COMP_LEVEL,
                # shuffle=COMP_SHUFFLE,
            )
            longitude_direct[:] = satobj.longitudes
            longitude_direct.long_name = "Longitude (Direct Georeferencing)"
            ...
            print("[ERROR] Unable to write direct latitude and longitude information to NetCDF file. L1c may be incomplete. Please run geometry computations.")
        ...
        try:
            ...
            print("[ERROR] Unable to write navigation angles to NetCDF file. L1c file may be incomplete. Please run geometry computations.")
            print("[ERROR] Encountered exception: " + str(ex))

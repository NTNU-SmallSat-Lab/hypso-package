...
def compute_elevation_angle(image_pos, sat_pos):
    ...
    viewpoint_mid_latlon_geocentric = np.array([m.atan2(R_eq*image_pos[2], R_pl*m.sqrt(image_pos[0]**2 + image_pos[1]**2)), \
...
def compute_local_angles(times, pos_teme, latlons):
    ...
    for i in range(times.shape[0]):
        ...
        location_astropy = astropy.coordinates.EarthLocation(lat=latlons[i,0]*180/m.pi*astropy.units.deg, lon=latlons[i,1]*180/m.pi*astropy.units.deg, height=0*astropy.units.m)
        ...
        sat_altaz = pos_astropy_teme.transform_to(astropy.coordinates.AltAz(obstime=time_astropy, location=location_astropy))
...
def compute_local_angles_2(times, pos_teme, lats, lons, indices):
    ...
    for i in range(times.shape[0]):
        ...
        for j,k in enumerate(indices):
            ...
            pixel_location_astropy = astropy.coordinates.EarthLocation(lat=lats[i,k]*astropy.units.deg, lon=lons[i,k]*astropy.units.deg, height=0*astropy.units.m)
...
def get_wkt(top_latlon, bot_latlon):
    '''
    returns a long string in Well known text representation of shapes format
    '''
    string = 'LINESTRING('
...
def add_grid(axes, lat_center, lon_center, seg_lat, seg_lon, size_lat_deg, size_lon_deg):
    ...
    for i in range(seg_lon+1):
        ...
        if i != 0:
            axes.text(x_grid_lat[0], y_grid_lat[0], z_grid_lat[0], f'{grid_lons[i]*180/m.pi:4.1f}', zdir, color='#DD7777')
    ...
    for i in range(seg_lat+1):
        ...
        axes.text(x_grid_lon[0], y_grid_lon[0], z_grid_lon[0], f'{grid_lats[i]*180/m.pi:4.1f}', zdir, color='#7777DD')
...
#    '''
    ...
    view_dir_0_top = gref.rotate_axis_angle(body_z_itrs[0,:], body_x_itrs[0,:], -fov_half*(hypso_height/hypso_height_sensor)*m.pi/180.0)
    view_dir_0_bot = gref.rotate_axis_angle(body_z_itrs[0,:], body_x_itrs[0,:],  fov_half*(hypso_height/hypso_height_sensor)*m.pi/180.0)
    ...
    view_dir_N_top = gref.rotate_axis_angle(body_z_itrs[-1,:], body_x_itrs[-1,:], -fov_half*(hypso_height/hypso_height_sensor)*m.pi/180.0)
    view_dir_N_bot = gref.rotate_axis_angle(body_z_itrs[-1,:], body_x_itrs[-1,:],  fov_half*(hypso_height/hypso_height_sensor)*m.pi/180.0)
    ...
    ax.plot3D([p1[0], p2[0], p4[0], p3[0], p1[0]], [p1[1], p2[1], p4[1], p3[1], p1[1]], [p1[2], p2[2], p4[2], p3[2], p1[2]], linewidth=0.8)
    ...
    for i in range(0,lines+1):
        ...
        ax.plot3D([view_start[0], view_end[0]],[view_start[1], view_end[1]],[view_start[2], view_end[2]], color='#222222', linewidth=0.8)
    ...
    subsample_pixels_indices = [ 0, hypso_height//4 - 1, hypso_height//2 - 1, 3*hypso_height//4 - 1, hypso_height - 1]

    local_angles_summary, allangles = compute_local_angles_2(times, pos_teme, pixels_lat, pixels_lon, subsample_pixels_indices)
    df = pd.DataFrame(local_angles_summary, columns=['Frame index', 'Frame time', 'latitude [degrees]', 'longitude [degrees]', 'Solar Zenith Angle [degrees]', 'Solar Azimuth Angle [degrees]', 'Satellite Zenith Angle [degrees]', 'Satellite Azimuth Angle [degrees]'])

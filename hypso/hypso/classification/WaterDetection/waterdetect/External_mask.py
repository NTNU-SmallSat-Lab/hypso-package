...
def search_mask(img_path: Path, masks_path: Path, img_type: str = 'S2_S2COR',
                msk_type: str = 'S2_S2COR') -> Union[Path, None]:
    """
    ...
    """
    img_detail = DWutils.parse_sat_name(img_path.stem, img_type=img_type)

    if img_detail is not None:
        for mask in masks_path.iterdir():
            mask_detail = DWutils.parse_sat_name(mask.name, img_type=msk_type)
...
def prepare_external_masks(imgs_dir: str, masks_dir: str, flags: list, img_type: str = 'S2_S2COR',
                           msk_type: str = 'S2_S2COR', dilation=0) -> None:
    ...
    for img in imgs:
        try:
            mask = search_mask(img, masks_path, img_type, msk_type)
            if mask is not None:
                print(f'Processing mask: {str(mask)}')
                process_mask(mask, img, flags, dilation=dilation)
            else:
                print(f'*** No mask found for img {img.stem}')

...
def decode_svm_labels(file_path: Path, spatial_dimensions: tuple):
...
def decode_svm_labels_masks(file_path: Path, spatial_dimensions: tuple) -> tuple[np.ndarray, np.ndarray, np.ndarray]:

    data = decode_svm_labels(file_path=file_path, spatial_dimensions=spatial_dimensions)

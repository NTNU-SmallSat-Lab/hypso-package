...
def load_l1d_nc(nc_file_path: Path, load_cube: bool = False) -> Tuple[dict, dict, dict, dict, dict, dict, np.ndarray]:
    ...
    nc_cube_attrs = load_l1d_nc_cube_attrs(nc_file_path=nc_file_path)

    nc_global_metadata = load_l1d_global_nc_metadata(nc_file_path=nc_file_path)

    nc_gcp_vars, nc_gcp_attrs = load_l1d_nc_gcp(nc_file_path=nc_file_path)
...
def load_l1d_nc_cube(nc_file_path: Path) -> np.ndarray:
    """
    Get Raw Cube from Hypso l1d.nc File

    :param nc_file_path: Absolute path to l1d.nc file
    ...
    with nc.Dataset(nc_file_path, format="NETCDF4") as f:
        ...
        try:
            ...
            cube = np.array(group.variables["rhot"][:], dtype='double')
        except Exception as ex:
            print("[INFO] Loading rhot cube from separate bands...")
...
def load_l1d_nc_cube_attrs(nc_file_path: Path) -> np.ndarray:
    """
    Get Raw Cube from Hypso l1d.nc File
    ...
    with nc.Dataset(nc_file_path, format="NETCDF4") as f:
        ...
        try:
            ...
            nc_cube_attrs["units"] = ""
            nc_cube_attrs["long_name"] = "Top-of-Atmosphere Reflectance"
            ...
            for idx, rhot_band in enumerate(list(group.variables)):
                ...
                subgroup = f.groups["products"][rhot_band]
...
def load_l1d_global_nc_metadata(nc_file_path: Path):
...
def load_l1d_nc_metadata(nc_file_path: Path) -> Tuple[dict, dict]:
    """
    Load l1d.nc Hypso Capture file metadata

    :param nc_file_path: Absolute path to the l1d.nc file
...
def load_l1d_nc_gcp(nc_file_path: Path) -> Tuple[dict, dict]:
    """
    Load l1d.nc Hypso Capture file GCPs

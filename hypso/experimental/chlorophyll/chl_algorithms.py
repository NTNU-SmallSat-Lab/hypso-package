...
#from hypso.experimental.chlorophyll.utilities_chl import safe_div, transformation
...
#             divide_condition = np.logical_or(r555 == 0, r555 == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r412, r555))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, r555))
#             div3 = np.where(divide_condition, np.nan, safe_div(r488, r555))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#
...
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, r555))
#             div3 = np.where(divide_condition, np.nan, safe_div(r488, r555))
#             div4 = np.where(divide_condition, np.nan, safe_div(r531, r555))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#             mbr = np.maximum(mbr, div4)
...
#             divide_condition = np.logical_or(denominator_rrs == 0, denominator_rrs == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r412, denominator_rrs))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, denominator_rrs))
#             div3 = np.where(divide_condition, np.nan, safe_div(r488, denominator_rrs))
#             div4 = np.where(divide_condition, np.nan, safe_div(r531, denominator_rrs))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#             mbr = np.maximum(mbr, div4)
...
#             # Calculate Chl OCX (O'Reilly et al., 1998)
#     with np.errstate(divide='ignore'):
#         lmbr = np.where(np.logical_or(mbr == 0, mbr == np.nan), np.nan, transformation(mbr))
#
#         chl_ocx = calculate_chl_ocx(ocx_poly, lmbr)
#         # Calculate Chl CI (Hu et al., 2012)
#         CI = rrs_green - (rrs_blue + (lambda_green - lambda_blue) / (lambda_red - lambda_blue) * (rrs_red - rrs_blue))
...
#         divide_condition = np.logical_or(denominator_rrs == 0, denominator_rrs == np.nan)
#
#         div = np.where(divide_condition, np.nan, safe_div(r416, denominator_rrs))
#         div2 = np.where(divide_condition, np.nan, safe_div(r444, denominator_rrs))
#         div3 = np.where(divide_condition, np.nan, safe_div(r490, denominator_rrs))
#         div4 = np.where(divide_condition, np.nan, safe_div(r513, denominator_rrs))
#
#         mbr = np.maximum(div, div2)  # Calculate max band ratio
#         mbr = np.maximum(mbr, div3)
#         mbr = np.maximum(mbr, div4)
...
#         with np.errstate(divide='ignore'):
#             divide_condition = np.logical_or(r560 == 0, r560 == np.nan)
#             div = np.where(divide_condition, np.nan, safe_div(r443, r560))
#             div2 = np.where(divide_condition, np.nan, safe_div(r490, r560))
#             div3 = np.where(divide_condition, np.nan, safe_div(r510, r560))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#
...
#             divide_condition = np.logical_or(r560 == 0, r560 == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r413, r560))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, r560))
#             div3 = np.where(divide_condition, np.nan, safe_div(r490, r560))
#             div4 = np.where(divide_condition, np.nan, safe_div(r510, r560))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#             mbr = np.maximum(mbr, div4)
...
#             divide_condition = np.logical_or(denominator_rrs == 0, denominator_rrs == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r413, denominator_rrs))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, denominator_rrs))
#             div3 = np.where(divide_condition, np.nan, safe_div(r490, denominator_rrs))
#             div4 = np.where(divide_condition, np.nan, safe_div(r510, denominator_rrs))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#             mbr = np.maximum(mbr, div4)
...
#             divide_condition = np.logical_or(r555 == 0, r555 == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r432, r555))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, r555))
#             div3 = np.where(divide_condition, np.nan, safe_div(r488, r555))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
...
#                 index_rexp = closest_index(wl, i)
#                 rexp = hypercube[:, :, index_rexp]
#                 divexp = np.where(divide_condition, np.nan, safe_div(rexp, r555))
#
#                 mbrxx = np.maximum(div, div2)  # Calculate max band ratio
#                 mbrxx = np.maximum(mbrxx, div3)
#                 mbrxx = np.maximum(mbrxx, divexp)
...
#             divide_condition = np.logical_or(denominator_rrs == 0, denominator_rrs == np.nan)
#
#             div = np.where(divide_condition, np.nan, safe_div(r432, denominator_rrs))
#             div2 = np.where(divide_condition, np.nan, safe_div(r443, denominator_rrs))
#             div3 = np.where(divide_condition, np.nan, safe_div(r488, denominator_rrs))
#             div4 = np.where(divide_condition, np.nan, safe_div(r500, denominator_rrs))
#             div5 = np.where(divide_condition, np.nan, safe_div(r531, denominator_rrs))
#
#             mbr = np.maximum(div, div2)  # Calculate max band ratio
#             mbr = np.maximum(mbr, div3)
#             mbr = np.maximum(mbr, div4)
...
#                 index_rexp = closest_index(wl, i)
#                 rexp = hypercube[:, :, index_rexp]
#                 divexp = np.where(divide_condition, np.nan, safe_div(rexp, denominator_rrs))
#
#                 mbrxx = np.maximum(div, div2)  # Calculate max band ratio
#                 mbrxx = np.maximum(mbrxx, div3)
#                 mbrxx = np.maximum(mbrxx, div4)
